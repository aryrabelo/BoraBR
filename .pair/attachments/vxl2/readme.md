...
> [!NOTE]
> <a href="https://pair.w3dev.fr"><img src="https://pair.w3dev.fr/logo-light.png" alt="PaiR" height="32"></a>
>
> **Looking for a standalone issue tracker with no external dependencies?**
>
> This app requires an external Beads CLI (`bd` or `br`) and follows its evolution. As the Beads ecosystem moves toward Dolt-only storage, server mode, and multi-agent orchestration, maintaining compatibility has become increasingly difficult.
>
> **[PaiR](https://pair.w3dev.fr)** is its successor — same philosophy (human in the loop, visual control, AI-native), but fully independent: built-in CLI, own storage format, zero external dependencies.
>
> **Migration to PaiR** is automatic for projects using `bd` 0.49.x (pre-Dolt) or `br` up to 0.1.x — these use the SQLite + JSONL format that PaiR reads natively. Projects running `bd` 0.50+ (Dolt-based) are not currently supported for migration.
>
